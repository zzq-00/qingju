      ...
      <div class="project-info">
        ...
        <div class="info">
          <ul class="info-list">
            ...
            <li>
              <span>服务内容：</span>
              <span>{{engineer.professional}}</span>
            </li>
            <li>
              <span>建筑面积：</span>
              <span>{{engineer.buildingArea}}㎡</span>
            </li>
            <li>
              <span>专业类型：</span>
              <span>{{engineer.professionalType}}</span>
            </li>
            <li>
              <span>是否提供CAD图纸：</span>
              <span>{{engineer.provideCad}}</span>
            </li>
            <li>
              <span>交付截止日期：</span>
              <span>{{engineer.deliveryDeadline}}</span>
            </li>
            <li v-if="engineer.localeWork">
              <span>是否需要现场办公：</span>
              <span>{{baseDto.officeAddress}}</span>
            </li>
            <li>
              <span>项目地址：</span>
            ...
            <li>
              <span>邀请码：</span>
              <span>{{baseDto.friendInvitationCode}}</span>
            </li>
          </ul>
        </div>
      </div>
      ...
      <div class="project-info" ref="appeal" v-if="this.$route.query.appealType || projectAppealList.length">
        ...
        <div class="file" v-if="showAppealSubmit">
          <div class="clearfix">
            ...
            <div>
              <label style="margin-right: 50px;" v-show="!engineer.detailStatusn">
                <input type="checkbox" :value="engineer" v-model="AppealProfessionType" @change="appealDeposit"> {{engineer.professional}}
              </label>
            </div>
          </div>
          <div class="clearfix" v-if="ComplaintParams.appealType==2">
            ...
            <div>
              <span style="color: #f00;">¥ {{depositNum | formatMoney}}</span>
...
export default {
  ...
  computed: {
    ...
    showAppealSubmit() {
      // 申诉之后detailStatusn有值
      let hasNoAppealType = !this.engineer.detailStatusn
      return !this.hasRepay && this.ComplaintParams.appealType && this.periodOfValidity < 8 && hasNoAppealType
    }
  },
  data() {
    return {
  ...
  methods: {
    ...
    // 查看项目详情
    viewEstablishment(id = this.$route.params.id) {
      api.viewEstablishment(id).then(res => {
        ...
        this.engineer = res.engineers[0] // 10概算编制 或者 9预算编制
        if (this.engineer.professional === '10') this.engineer.professional = '概算编制'
        if (this.engineer.professional === '9') this.engineer.professional = '预算编制'
    ...
    check() {
      this.$confirm('您是否已经确认验收成果文件？', '提示', {
        ...
        .then(() => {
          api
            .check({ couponId: '', projectBaseId: this.projectBaseId })
            .then(res => {
              this.$message({
                message: res.errorMsg,
                type: 'success'
