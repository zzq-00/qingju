<template>
  <div class="project">
    <p class="page-title">所有项目</p>
    <filterComponent @handleFilter="handleFilter" @download="download" :placeholder="'项目名、发布人'"></filterComponent>
    <el-table ref="multipleTable" :data="tableData" v-loading="loading" class="myQingju-table" @selection-change="changeFun" @select="selectMemoryFn" @select-all="selectAll">
      <el-table-column width="35" label="213" type="selection" class-name='selection-self'>
      </el-table-column>
      <el-table-column label="项目信息" prop="" min-width="300">
        <template slot-scope="scope">
          <p class="project-name" @click="gotoDetail(scope.row.projectBaseId,scope.row.projectBaseType,scope.row.publishTime)">{{scope.row.name}}</p>
          <!-- <p v-else>{{scope.row.name}}</p> -->
          <p>
            <!-- , query: {userId: JSON.parse(localStorage.QingjuuserId) -->
            <router-link style="color: #00b38a;margin-right: 10px;" :to="{path: '/myqingju/userDetails', query: {userId: scope.row.userId}}" target="_blank">
      ...
      <el-table-column label="项目类型" prop="" :render-header="renderType" min-width="130">
        <template slot-scope="scope">
          <div class="type">
            <p class="type-text" v-for="item in scope.row.engineers" :key="item.projectInfoId" :style="{backgroundColor: computedBackgroundColor(item.professional).color}">
              {{computedBackgroundColor(item.professional).name}}</p>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="项目金额" prop="amount" :render-header="renderSort" min-width="130"></el-table-column>
      <el-table-column label="完成时间" prop="checkedTime" :render-header="renderSort" min-width="130">
        <!-- <template slot-scope="scope">
      ...
      <el-table-column label="项目状态" prop="status" :render-header="renderStatus" min-width="130">
        <template slot-scope="scope">
          <span class="status-text" :style="{color: computedColor(scope.row.status)}">
            {{scope.row.status}}
          </span>
        </template>
      ...
      <el-table-column label="操作" width="120">
        <template slot-scope="scope">
          <div class="more-handle more-handle-green" v-if="scope.row.userId === rootId">
            ...
            <p v-if="(scope.row.projectBaseType === '1' || scope.row.projectBaseType === '5') && (scope.row.status === '已完成' || scope.row.status === '待验收')" @click="showAppeal(scope.row)">发起申诉</p>
            <p v-if="scope.row.status === '已完成'">
              <router-link :to="'/myqingju/evaluation/'+scope.row.projectBaseId" target="_blank">{{scope.row.evaluateStatus === '未评价' ? '评价': '已评价'}}</router-link>
            </p>
            <p v-if="scope.row.status === '待托管' || scope.row.isThereAnySurcharge" @click="gotoDetail(scope.row.projectBaseId,scope.row.projectBaseType,scope.row.publishTime)">托管资金</p>
            <p v-if="scope.row.status === '待验收'" @click="gotoDetail(scope.row.projectBaseId,scope.row.projectBaseType,scope.row.publishTime)">验收项目</p>
          ...
          <div class="more-handle more-handle-green">
            ...
            <p @click="reEdit(scope.row, 'reuse')">使用该模板</p>
          </div>
        </template>
      </el-table-column>
    ...
    <div class="pagination-layout">
      <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page.sync="currentPage" :page-sizes="[10, 20, 30, 50, 100]" :page-size="20" layout="total, sizes, prev, pager, next, jumper" :total="total" border>
      </el-pagination>
    </div>
    <el-dialog title="请选择申诉类型" :visible.sync="dialogVisible" width="550px">
      <ul class="appeal-list">
        <li @click="gotoDetail(appealData.projectBaseId,appealData.projectBaseType,appealData.publishTime, '?appealType=1#appeal')">
          <img src="@/assets/img/price-appeal.png" alt="">申诉项目价格
        </li>
        <li @click="gotoDetail(appealData.projectBaseId,appealData.projectBaseType,appealData.publishTime, '?appealType=2#appeal')">
          <img src="@/assets/img/quality-appeal.png" alt="">申诉项目质量
        </li>
      </ul>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    toApplyBill(row) {
      let obj = {
        name: row.name,
        actualProjectCost: row.amount,
        projectBaseId: row.projectBaseId
      }
      window.localStorage.proArr = JSON.stringify([obj])
    ...
    computedBackgroundColor(item) {
      switch (item) {
        case '土建工程':
          return { name: '土建', color: '#ff5e5b' }
        case '安装工程':
          return { name: '安装', color: '#00b38b' }
        case 'BIM':
          return { name: 'BIM', color: '#46b620' }
        case '青矩设计':
          return { name: '设计', color: '#cc0066' }
        case '智能建模':
        case '结构智能建模':
          return { name: '智能建模', color: '#33cccc' }
        case '安装智能建模':
          return { name: '安装智能建模', color: '#CC6633' }
        case '土建智能建模':
          return { name: '土建智能建模', color: '#33cccc' }
        case '概算编制':
        case '10':
          return { name: '概算编制', color: '#660033' }
        case '预算编制':
        case '9':
          return { name: '预算编制', color: '#990033' }
        default:
          return { name: '', color: '#009966' }
      }
    },
    gotoDetail(baseId, baseType, publishTime, anchor = '') {
      let routerPath
      // 兼容2019年7月18日发布前的旧项目
      if (new Date(publishTime) < new Date('2019-07-18') && baseType === '4') {
        routerPath = this.$router.resolve('/myqingju/modelingDetail/' + baseId)
        return window.open(routerPath.href, '_blank')
      }

      if (baseType === '1' || baseType === '4') {
        routerPath = this.$router.resolve('/myqingju/projectDetail/' + baseId + anchor)
      } else if (baseType === '2') {
        routerPath = this.$router.resolve('/myqingju/BIMDetail/' + baseId)
      } else if (baseType === '3') {
        routerPath = this.$router.resolve('/myqingju/designDetail/' + baseId)
      } else if (baseType === '5') {
        routerPath = this.$router.resolve('/myqingju/establishmentDetail/' + baseId + anchor)
      }
    ...
    // 重新编辑
    reEdit(data, flag = '') {
      if (data.projectBaseType === '1' || data.projectBaseType === '4') {
        // 计量
        this.$router.push({
          path: '/businessService/releaseEstimate?reEditId=' + data.projectBaseId,
          query: {
            flag: flag
          }
        })
      } else if (data.projectBaseType === '2') {
        // BIM
        this.$router.push({
          path: '/businessService/releaseBIM?reEditId=' + data.projectBaseId,
          query: {
            flag: flag
          }
        })
      } else if (data.projectBaseType === '3') {
        // 设计
        this.$router.push({
          path: '/businessService/releaseDesign?reEditId=' + data.projectBaseId,
          query: {
            flag: flag
          }
        })
      } else if (data.projectBaseType === '5') {
        // 编制
        this.$router.push({
          path: '/businessService/releaseEstablishment?reEditId=' + data.projectBaseId,
          query: {
            flag: flag
          }
        })
    ...
    selectAll(selection) {
      ...
      if (!this.ids.length || (this.indeterminate && this.ids.length)) {
        // 全选
    ...
    selectMemoriedDataFn() {
      ...
      if (this.allTableData && this.allTableData.length !== this.ids.length && this.ids.length) {
        this.indeterminate = true
      }
      this.setCurrent()
      // 选择框样式
    ...
    setCurrent() {
      ...
      if (currentArr && currentArr.length) {
        ...
        currentArr.forEach((item, index) => {
          // 遍历当前分页被选中数据
          if (tempRowsIDs.includes(item.projectBaseId)) {
            // id匹配上，则选中
