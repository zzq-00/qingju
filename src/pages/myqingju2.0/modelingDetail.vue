  ...
  <div>
    ...
    <div class="container">
      ...
      <div class="status clearfix">
        <div class="left fl">
          <span class="font12">青矩编号:{{robotProjectBaseDto.projectNumber}}</span>
          <p>{{robotProjectBaseDto.statusName}}</p>
          <p v-if="robotProjectBaseDto.status==3">¥ {{robotProjectBaseDto.amount | formatMoney}}</p>
          <button @click="payAmount" v-if="robotProjectBaseDto.status==3">托管资金</button>
          <button @click="modelingCheck" v-if="robotProjectBaseDto.status==6">确认验收</button>
          <button @click="evaluation" v-if="robotProjectBaseDto.status==7">{{evaluationOfState?'已评价':'评价'}}</button>
          <div>
            <span class="confirm" v-if="robotProjectBaseDto.status>=3&&robotProjectBaseDto.status<8" @click="dialogVisibleTrue('view')">{{ifSign?'查看合同':'签约合同'}}</span>
            <span class="download" v-if="robotProjectBaseDto.status>=3&&robotProjectBaseDto.status<8" @click="dialogVisibleTrue('download')" title="下载后，请盖章后寄送给我们">下载合同模板</span>
          </div>
        </div>
        <div class="right fl" v-if="robotProjectBaseDto.status<8">
          <div>
            <span class="font12" v-if="robotProjectBaseDto.status==1">感谢您使用青矩互联平台，我们会尽快审批该项目，请您耐心等候！</span>
            <span class="font12" v-if="robotProjectBaseDto.status==2">感谢您使用青矩互联平台，该项目已经审批通过，正在等待确认，请您耐心等候！</span>
            <span class="font12" v-if="robotProjectBaseDto.status==3">该项目会为您保留30个工作日，请尽快托管资金，感谢您使用青矩互联平台！</span>
            <span class="font12" v-if="robotProjectBaseDto.status==4">您已经付款成功，项目承接部门会马上开始工作，请您耐心等候！</span>
            <span class="font12" v-if="robotProjectBaseDto.status==5">您的项目已经开始工作，请您耐心等候！</span>
            <span class="font12" v-if="robotProjectBaseDto.status==6">您项目的成果文件已经提交，请您尽快验收！</span>
            <span class="font12" v-if="robotProjectBaseDto.status==7">恭喜您，您的项目已经完成交付。</span>
            <!-- <span class="font12" v-if="robotProjectBaseDto.status==3"><img src="../../assets/img/u774.png" alt="" style="vertical-align: bottom;"> 剩余时间：{{robotProjectBaseDto.paymentCountdown}}</span> -->
          ...
          <div class="step">
            <div style="color: #333;">
              ...
              <span>已发布</span>
              <span>{{robotProjectBaseDto.publishTime}}</span>
            </div>

            <img v-if="robotProjectBaseDto.status>1" src="../../assets/img/u667.png" alt="">
            <img v-else src="../../assets/img/u378.png" alt="">

            <div v-if="robotProjectBaseDto.status>=2" style="color: #333;">
              <img src="../../assets/img/u1996.png" alt="">
              <span>已审批</span>
              <span>{{robotProjectBaseDto.pendingTime}}</span>
            </div>
            <div v-else>
            ...
            <img v-if="robotProjectBaseDto.status>2" src="../../assets/img/u667.png" alt="">
            <img v-else-if="robotProjectBaseDto.status==2" src="../../assets/img/u378.png" alt="">
            <img v-else src="../../assets/img/u379.png" alt="">

            <div v-if="robotProjectBaseDto.status>=3" style="color: #333;">
              <img src="../../assets/img/u1997.png" alt="">
              <span>已确认</span>
              <span>{{robotProjectBaseDto.confirmTime}}</span>
            </div>
            <div v-else>
            ...
            <img v-if="robotProjectBaseDto.status>3" src="../../assets/img/u667.png" alt="">
            <img v-else-if="robotProjectBaseDto.status==3" src="../../assets/img/u378.png" alt="">
            <img v-else src="../../assets/img/u379.png" alt="">

            <div v-if="robotProjectBaseDto.status>=4" style="color: #333;">
              <img src="../../assets/img/u2131.png" alt="">
              <span>已托管</span>
              <span>{{robotProjectBaseDto.trusteeshipTime}}</span>
            </div>
            <div v-else>
            ...
            <img v-if="robotProjectBaseDto.status>5" src="../../assets/img/u667.png" alt="">
            <img v-else-if="robotProjectBaseDto.status==4 || robotProjectBaseDto.status==5" src="../../assets/img/u378.png" alt="">
            <img v-else src="../../assets/img/u379.png" alt="">

            <div v-if="robotProjectBaseDto.status>=6" style="color: #333;">
              <img src="../../assets/img/u2831.png" alt="">
              <span>已交付</span>
              <span>{{robotProjectBaseDto.deliveryTime}}</span>
            </div>
            <div v-else>
            ...
            <img v-if="robotProjectBaseDto.status>6" src="../../assets/img/u667.png" alt="">
            <img v-else-if="robotProjectBaseDto.status==6" src="../../assets/img/u378.png" alt="">
            <img v-else src="../../assets/img/u379.png" alt="">

            <div v-if="robotProjectBaseDto.status>=7" style="color: #333;">
              <img src="../../assets/img/u3073.png" alt="">
              <span>已完成</span>
              <span>{{robotProjectBaseDto.checkedTime}}</span>
            </div>
            <div v-else>
        ...
        <div class="right fl" v-else>
          <div>
            <span class="font12" v-if="robotProjectBaseDto.activeClose">您取消了项目，感谢您使用青矩互联平台！</span>
            <span class="font12" v-else-if="robotProjectBaseDto.status==='8'">您的项目已被关闭，感谢您使用青矩互联！</span>
            <span class="font12" v-else-if="robotProjectBaseDto.status==='9'">您的项目暂时被驳回了，感谢您使用青矩互联！</span>
          </div>
          <div class="step" v-if="robotProjectBaseDto.activeClose">
            <div>
            ...
            <div>
              ...
              <span>取消支付</span>
              <span>{{robotProjectBaseDto.closeTime}}</span>
            </div>
            <img src="../../assets/img/u379.png" alt="">
            <div>
          ...
          <div class="step" v-else>
            ...
            <template v-if="robotProjectBaseDto.status === '9'">
              <img src="@/assets/img/u379.png" alt="">
              <div>
                <img src="@/assets/img/u3358.png" alt="">
                <span>已驳回</span>
      ...
      <!-- 成果文件 -->
      <div class="project-info" v-if="projectProcesses && projectProcesses.length>0 &&robotProjectBaseDto.status==='6'">
        <div>成果文件</div>
      ...
      <!-- 项目验收 -->
      <div class="project-info" v-if="robotProjectBaseDto.status === '6'">
        <div>项目验收</div>
        ...
        <div class="price-calculation">
          ...
          <ul>
            ...
            <li v-for="(item,index) in couponList" :key="'02'+index" v-if="checkParams.couponId === item.id">
              <span>应付金额：</span>
              <b>¥ {{(setupCost-item.subtractLimit > 0)?(setupCost-item.subtractLimit):0 | formatMoney}}</b>
            </li>
          </ul>
        </div>
        <div class="pay-button">
          <button class="pay" @click="modelingCheck">确认验收</button>
        </div>
      </div>
      <!-- 待付款费用信息 -->
      <div class="project-info" v-if="notPayServiceBills && notPayServiceBills.length>0">
        ...
        <div class="exponent-price">
          <div>应托管金额：
            <b>¥ {{robotProjectBaseDto.amount | formatMoney}}</b>
          </div>
        </div>
        <div class="pay-button">
      ...
      <div class="project-info" v-if="notPaySurcharges && notPaySurcharges.length>0">
        ...
        <div class="exponent-price">
          <div>支付金额：
            <b style="color:#FF0000;font-size:20px;">¥ {{robotProjectBaseDto.amount | formatMoney}}</b>
          </div>
        </div>
        <div class="pay-button">
          <button class="pay" @click="payAmount">去收银台支付</button>
      ...
      <div class="project-info">
        ...
        <div class="info">
          <ul class="info-list">
            <li>
              <span>项目名称：</span>
              <span>{{robotProjectBaseDto.projectName}}</span>
            </li>
            <li>
              <span>服务内容：</span>
              <span>{{engineers.serviceContent}}</span>
            </li>
            <li>
              <span>建筑类型：</span>
              <span v-if="engineers.type">{{engineers.type}}</span>
              <span v-else>其他：{{engineers.typeOther}}</span>
            </li>
            <li>
              <span>建筑面积：</span>
              <span>{{engineers.buildingArea}} ㎡</span>
            </li>
            <li>
              <span>单体个数：</span>
              <span>{{engineers.monomerNumber}} </span>
            </li>
            <li>
              <span>专业类型：</span>
              <span>{{engineers.professionalType}}</span>
            </li>
            <li v-show="engineers.dataModel">
              <span>是否提供混凝土工程量：</span>
              <span>{{engineers.dataModel}}</span>
            </li>
            <li>
              <span>委托范围：</span>
              <span>{{engineers.commissionScope}}</span>
            </li>
            <li>
              <span>交付截止日期：</span>
              <span>{{engineers.deliveryDeadline}}</span>
            </li>
            <li>
              <span>项目地址：</span>
              <span>{{engineers.location}}</span>
            </li>
            <li>
              <span>联系电话：</span>
              <span>{{robotProjectBaseDto.mobile}}</span>
            </li>
            <li>
              <span>附件：</span>
              <span>
                <div v-for="(item,index) in robotProjectBaseDto.files" :key="index">
      ...
      <!-- 成果文件 -->
      <div class="project-info" v-if="projectProcesses && projectProcesses.length>0 &&robotProjectBaseDto.status==='7'">
        <div>成果文件</div>
        <el-table :data="projectProcesses" style="width: 100%">
          <el-table-column prop="deliveryTime" label="交付时间" width="180">
          </el-table-column>
      ...
      <!-- 项目申诉 -->
      <div class="project-info" ref="path" v-if="(robotProjectBaseDto.status==6 && !hasRepay) || (robotProjectBaseDto.status==7 && !hasRepay)">
        <div>项目申诉</div>
        <div class="file" v-if="periodOfValidity<8 && !projectAppealList.length">
          <!-- <div class="clearfix">
            <label class="require fl">申诉类型：</label>
            <div>
              <label style="margin-right: 50px;" v-for="(item,index) in engineers" :key="index" v-show="!item.detailStatusn">
                <input type="checkbox" :value="item.professional" v-model="ComplaintParams.type"> {{item.professional}}
              </label>
            </div>
          </div> -->
          <div class="clearfix">
        ...
        <p style="text-align: center;padding: 20px 0;" v-if="periodOfValidity>7">暂无申诉</p>
      </div>
      <!-- 补充文件 -->
      <div class="project-info add-file" v-if="robotProjectBaseDto.status<8">
        <div>补充文件</div>
        <div class="file">
          <div class="clearfix">
      ...
      <!-- 留言，问题描述 -->
      <div class="question" v-if="robotProjectBaseDto.status<8">
    ...
    <el-dialog :title="doWhat==='download'?'下载合同模板':'合同签约'" :visible.sync="dialogVisible" width="30%" class="dialog">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px">
        ...
        <el-form-item style="margin-bottom:0">
          ...
          <div style="margin-top: 10px"> <span v-if="prompt"> {{promptMes}}</span><span v-if="verify">为了保证您签约的电子合同与纸质版的合同具有相同法律效应，使用此服务
...
export default {
  ...
  methods: {
    payAmount() {
      window.open('/myqingju/cashierDesk/modeling/' + this.$route.params.id)
    },
    gotoPage() {
    ...
    // 查看项目详情
    viewRobotProject(id = this.$route.params.id) {
      api.viewRobotProject(id).then(res => {
        this.robotProjectBaseDto = res.robotProjectBaseDto
        this.engineers = res.engineers
        this.projectBaseId = res.robotProjectBaseDto.projectBaseId
        if (res.orderIds && res.orderIds.length > 0) this.robotProjectBaseDto.orderId = res.orderIds[0]
        this.robotProjectBaseDto.amount = res.amount // 托管付款金额
        this.robotProjectBaseDto.paymentCountdown = res.paymentCountdown // 剩余付款时间
        this.notPayServiceBills = res.notPayServiceBills // 待托管账单
        this.havePayBills = res.havePayBills // 已付款账单信息
        this.setupCost = res.setupCost // 项目已付
        this.actuallyPayAmount = res.actuallyPayAmount // 项目已付
        this.projectProcesses = res.projectProcesses // 成果文件
        this.evaluationOfState = res.robotProjectBaseDto.evaluationOfState // 是否已评价
        this.ifSign = res.robotProjectBaseDto.ifSign //是否已签约
        this.ContractAddress = res.robotProjectBaseDto.contractLink //签约地址
        ...
        this.ComplaintParams.type[0] = res.engineers.professional // 申诉类型
        if (this.robotProjectBaseDto.status === '6') this.getMyCouponByProjectId()
        if (this.robotProjectBaseDto.status === '6' || this.robotProjectBaseDto.status === '7') this.HasRepayQuickPayBill()
      })
    },
    // 是否已还过轻松付账单，还过将不能申诉
    ...
    projectComplaint() {
      ...
      if (this.ComplaintParams.type.length === 0) return this.$message('请选择申诉类型')
